...
import Carbon
import os.log
private let recordingLog = OSLog(subsystem: "com.jawsbb.vocali", category: "Recording")
...
final class AppState: ObservableObject, @unchecked Sendable {
    ...
    /// URL of the flag file written while Vocali is actively recording.
    ///
    ...
    ///
    /// Path: `~/Library/Application Support/Vocali/is-recording`
    /// (or `Vocali Dev/is-recording` when running the dev bundle).
    static func recordingStateFlagURL() -> URL {
        let appSupport = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask).first!
        let appName = Bundle.main.object(forInfoDictionaryKey: "CFBundleName") as? String ?? "Vocali"
    ...
    private static let recordingStateFlagQueue = DispatchQueue(
        label: "com.jawsbb.vocali.recording-state-flag"
    )

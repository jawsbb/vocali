APP_NAME ?= Vocali Dev
BUNDLE_ID ?= com.jawsbb.vocali.dev
BUILD_DIR = build
APP_BUNDLE = $(BUILD_DIR)/$(APP_NAME).app
CODESIGN_IDENTITY ?= Vocali Dev
...
# which Vocali they are running when both are installed side by side.
ifeq ($(APP_NAME),Vocali Dev)
...
ifeq ($(ARCH),universal)
	...
	@codesign --force --options runtime --sign "$(CODESIGN_IDENTITY)" --entitlements Vocali.entitlements "$(APP_BUNDLE)"
